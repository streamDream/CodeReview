...
class Redis(object):
    ...
    这个抽象类为所有Redis命令提供Python接口，同时为Redis协议提供实现

    Connection and Pipeline derive from this, implementing how
    the commands are sent and received to the Redis server
    Connection和Pipeline类派生至Redis类，它们实现了命令发送到Redis服务器和从Redis服务器接受命令的返回值
    """
    ...
    RESPONSE_CALLBACKS = dict_merge(
        ...
        string_keys_to_dict(
            ...
            'LPUSH RPUSH',
            lambda r: isinstance(r, (long, int)) and r or nativestr(r) == 'OK'  # TODO 这里是一个lambda表达式！！！
        ),
    ...
    def from_url(cls, url, db=None, **kwargs):
        ...
        """
        # TODO 仅仅创建了一个最简单的连接池对象（并没有建链）！！！
        connection_pool = ConnectionPool.from_url(url, db=db, **kwargs)

        # TODO cls==Redis   Redis(connection_pool=connection_pool)
        # 即调用下面的Redis.__init()__
    ...
    def __init__(self, host='localhost', port=6379,
        ...
        if not connection_pool:  # TODO 不用Redis.fromUrl()的方式得到redis对象时，connection_pool为空。
            # TODO 和使用Redis.fromUrl()的kwargs参数构造实现一样，构造出创建connection_pool对象所需要的参数。
        ...
        self.connection_pool = connection_pool  # TODO 创建连接池对象

        self.response_callbacks = self.__class__.RESPONSE_CALLBACKS.copy()  # TODO 返回值类型转化函数
    ...
    def pipeline(self, transaction=True, shard_hint=None):
        ...
        TODO 返回一个能编队多条命令后一次运行的新pipeline对象。
        transaction参数指明是否将所有的命令作为一次原子操作运行。
        除了产生一组原子性的操作外，pipeline能有效减少client和server之间的来回次数。
        """
        return Pipeline(
    ...
    def transaction(self, func, *watches, **kwargs):
        ...
        TODO 当WATCH所有<watches>中的值时，方便地事务化调用<func>。func可调用器只能有一个Pipeline对象作为参数。
        ...
        TODO
            def test_transaction(self):
                ...
                result = self._rs.transaction(f, *[], **dict(value_from_callable=True))
                print('>>>> result: ', result)
        """
        shard_hint = kwargs.pop('shard_hint', None)
        value_from_callable = kwargs.pop('value_from_callable', False)  # TODO 是否使用func的返回值作为返回值
        ...
        with self.pipeline(True, shard_hint) as pipe:
            while True:
                try:
                    if watches:
                        pipe.watch(*watches)
                    func_value = func(pipe)  # TODO Pipeline对象作为参数
    ...
    def execute_command(self, *args, **options):
        # TODO 所有单条命令数据报文的执行方法
        "Execute a command and return a parsed response"  # TODO 执行一条命令并返回一个解析过的响应

        pool = self.connection_pool
        command_name = args[0]
        connection = pool.get_connection(command_name, **options)
        try:
            connection.send_command(*args)  # TODO 通过connection对象发送命令
            return self.parse_response(connection, command_name, **options)  # TODO 读取同个connection的response
        except (ConnectionError, TimeoutError) as e:
            connection.disconnect()
            if not (connection.retry_on_timeout and
                    isinstance(e, TimeoutError)):
                raise
            connection.send_command(*args)  # TODO 先检查链接是否存在，不存在则建链
            return self.parse_response(connection, command_name, **options)
        finally:
            pool.release(connection)  # TODO response读取完成之后，将connection放到未使用的链接池中
...
class Pipeline(Redis):
    ...
    TODO Pipeline提供了一种通过一次传输就将多条命令发送到Redis server的方式。

    All commands executed within a pipeline are wrapped with MULTI and EXEC
    calls. This guarantees all commands executed in the pipeline will be
    executed atomically.
    TODO pipeline中所有的命令被MULTI和EXEC包装，这就保证了所有的命令的原子性。
    ...
    TODO 在pipeline中，任何命令抛出异常不会停止命令组的执行。
    抛出的异常会被捕获，异常的实例对象会被加到作为返回值列表中。
    迭代遍历返回值列表就可以处理异常的实例对象。
    通常这些异常会是ResponseError类的对象，像在命令的key的数据类型不正确时。
    """
    ...
    def reset(self):
        ...
        # watching something
        # TODO 确保正在watch的时候也能重置connection
        if self.watching and self.connection:
            try:
                # call this manually since our unwatch or
                # immediate_execute_command methods can call reset()
                # TODO 手动调用unwatch，因为unwatch或immediate_execute_command方法能够调用reset()
                self.connection.send_command('UNWATCH')
                self.connection.read_response()
            except ConnectionError:
                # disconnect will also remove any previous WATCHes
                # TODO 断链也会移出已经存在的所有watch
                self.connection.disconnect()
    ...
    def multi(self):
        ...
        if self.command_stack:
            raise RedisError('Commands without an initial WATCH have already '
                             'been issued')
        self.explicit_transaction = True  # TODO 标识已经调用过MULTI

    def execute_command(self, *args, **kwargs):
        # TODO 当已经处于watching状态或者显式地调用WATCH命令且没有调用过MULTI
    ...
    def immediate_execute_command(self, *args, **options):
        ...
        MULTI is called.
        # TODO 立即执行一条命令，在已WATCH一个变量的情况下抛出ConnectionError异常时不会重试。
        在MULTI前被调用，在WATCH或者查询命令组返回值时使用。
        """
        command_name = args[0]
    ...
    def pipeline_execute_command(self, *args, **options):
        ...
        """
        self.command_stack.append((args, options))  # TODO 将命令入栈
        return self

    def _execute_transaction(self, connection, commands, raise_on_error):
        # TODO 使用事务包装并执行所有的命令，然后解析响应
    ...
    def execute(self, raise_on_error=True):
        ...
        if self.scripts:
            self.load_scripts()
        if self.transaction or self.explicit_transaction:  # TODO 根据是否按照事务执行和显式调用过MULTI
