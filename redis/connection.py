...
class Token(object):
    ...
    def __init__(self, value):
        if isinstance(value, Token):
            value = value.value
        self.value = value  # TODO command的string类型值
        self.encoded_value = value.encode()  # TODO command的bytes类型值
...
class Encoder(object):
    ...
    # TODO 将strings编码为bytes，将bytes解码为strings

    def __init__(self, encoding, encoding_errors, decode_responses):
        self.encoding = encoding  # TODO 编码的类型，例如：utf-8
...
class SocketBuffer(object):
    def __init__(self, socket, socket_read_size):
        ...
        # TODO 写进buffer的bytes长度
        self.bytes_written = 0
        # number of bytes read from the buffer
        # TODO 从buffer已读取的bytes长度
        self.bytes_read = 0
    ...
    def _read_from_socket(self, length=None):
        ...
        buf = self._buffer
        buf.seek(self.bytes_written)  # TODO 指针跳到buffer中已写bytes的末尾
        marker = 0  # TODO 用来记录已经从socket中接收到的报文的总长度

        try:
            # TODO 如果length为None，则while循环仅仅只执行了1次
            while True:
                data = recv(self._sock, socket_read_size)  # TODO 从socket中读取指定长度的数据报文
                # an empty string indicates the server shutdown the socket
                if isinstance(data, bytes) and len(data) == 0:
                    raise socket.error(SERVER_CLOSED_CONNECTION_ERROR)
                buf.write(data)  # TODO 将读取到的response数据写到self._buffer
                data_length = len(data)
                self.bytes_written += data_length
                marker += data_length

                if length is not None and length > marker:  # TODO 当接收的报文长度小于length时，才继续接收
                    continue
                break
    ...
    def read(self, length):
        ...
        if length > self.length:
            self._read_from_socket(length - self.length)  # TODO 读取剩下的bytes

        self._buffer.seek(self.bytes_read) # TODO 指针跳到上次已经读取的位置末尾
        data = self._buffer.read(length)
        self.bytes_read += len(data)

        # purge the buffer when we've consumed it all so it doesn't
        # grow forever
        if self.bytes_read == self.bytes_written:  # TODO 已经读取到了指定的bytes
            self.purge()

        return data[:-2]  # TODO 舍弃末尾的SYM_CRLF

    def readline(self):
        # TODO 读取1条完整的数据，即该方法通过while循环来实现读取一个完整的response
        buf = self._buffer
        buf.seek(self.bytes_read)
        data = buf.readline()
        while not data.endswith(SYM_CRLF):  # TODO 没有到达response的CRLF标识，即未到尾部则继续
    ...
    def purge(self):
        # TODO 初始化buffer和读写位置标记，准备读取下一个响应
        self._buffer.seek(0)
        self._buffer.truncate() # TODO 截断buffer？？？
        self.bytes_written = 0
        self.bytes_read = 0

    def close(self):
        # TODO 彻底初始化一个socketBuffer
        try:
            self.purge()
            self._buffer.close()
        except Exception:
...
class PythonParser(BaseParser):
    ...
    def __init__(self, socket_read_size):
        # TODO 创建socket数据报文解析对象。未显式调用super().__init__()？？？
        self.socket_read_size = socket_read_size # TODO 读取的报文的长度
        self.encoder = None
        self._sock = None
        self._buffer = None

    def __del__(self):
        # TODO 对象销毁时finit self
        try:
            self.on_disconnect()
        except Exception:
            pass
    ...
    def on_disconnect(self):
        ...
        # TODO 断链时调用，finit自身
        self._sock = None  # TODO 清空socket链接对象
        if self._buffer is not None:
            self._buffer.close() # TODO 关闭报文buffer
            self._buffer = None
        self.encoder = None

    def can_read(self):
        return self._buffer and bool(self._buffer.length)  # TODO 有buffer且长度不为0
...
class HiredisParser(BaseParser):
    ...
    #     self._reader = hiredis.Reader(**kwargs)  # TODO 注册读取返回值的对象！！！
    #     self._next_response = False  # TODO 设置下一个可读的响应为False
    #
    # def on_disconnect(self):
    #     self._sock = None
    #     self._reader = None
...
# TODO 根据是否安装了hiredis-py来选择解析的class
if HIREDIS_AVAILABLE:
...
class Connection(object):
    ...
    def __init__(self, host='localhost', port=6379, db=0, password=None,
                 ...
                 parser_class=DefaultParser, socket_read_size=65536):  # TODO 默认读取的报文长度为65536！！！
        self.pid = os.getpid()
        self.host = host
        self.port = int(port)
        self.db = db
        ...
        self._sock = None
        self._parser = parser_class(socket_read_size=socket_read_size)  # TODO 初始化回复的解析class
    ...
    def pack_command(self, *args):
        ...
        buff = SYM_EMPTY.join((SYM_STAR, str(len(args)).encode(), SYM_CRLF))  # TODO bytes类型表示的args的长度
...
class ConnectionPool(object):
    ...
    def from_url(cls, url, db=None, decode_components=False, **kwargs):
        ...
        # TODO cls==ConnectionPool，此处调用ConnectionPool.__init__()
        # TODO 如果url是以"redis://"开头，那么kwargs中是不包含connection_class的。
        return cls(**kwargs)

    def __init__(self, connection_class=Connection, max_connections=None,
                 **connection_kwargs):
        """
        # TODO connection_class默认值为自定义的Connection！！！

        Create a connection pool. If max_connections is set, then this
        object raises redis.ConnectionError when the pool's limit is reached.
        # TODO 创建一个链接池对象。如果max_connections被赋值了，当连接池达到链接上限值后，这个对象将抛出redis.ConnectionError。
        ...
        # TODO 默认创建TCP链接，除非connection_class被指定了具体的值。对于unix sockets会使用redis.UnixDomainSocketConnection创建链接。

        Any additional keyword arguments are passed to the constructor of
        connection_class.
        # TODO 任何附加的关键字参数将被传递给connection_class的构造器（__init__()）。
        """
        ...
        self.reset()  # TODO 仅仅初始化几个对象属相，并没有建链！！！
    ...
    def reset(self):
        # TODO 初始化几个对象属性
        self.pid = os.getpid()  # TODO 主进程的PID
        self._created_connections = 0  # TODO 已经创建的链接数
        self._available_connections = []  # TODO 正在使用的链接数
        self._in_use_connections = set()  # TODO 正在使用的链接对象的集合
        self._check_lock = threading.Lock()  # TODO 线程锁对象

    def _checkpid(self):
        # TODO 这里主要是针对多线程同时初始化ConnectionPool中的connection池的并发控制
        if self.pid != os.getpid(): # TODO 子进程中调用
            with self._check_lock: # TODO 获取线程锁
                if self.pid == os.getpid():  # TODO 另外的一个线程进入之后直接返回
                    # another thread already did the work while we waited
                    # on the lock.
                    return
                self.reset() # TODO 子进程中调用时重置连接池

    def get_connection(self, command_name, *keys, **options):
        "Get a connection from the pool"
        self._checkpid()  # TODO 检查线程锁，解决多线程同时调用该方法时的并发问题
        try:
            connection = self._available_connections.pop()  # TODO 从list中弹出一个可用的链接
        except IndexError:
            connection = self.make_connection()  # TODO 没有可用链接时，创建1个链接
        self._in_use_connections.add(connection) # TODO 将创建的Connection对象放到可用链接对象list中
        try:
            # ensure this connection is connected to Redis
            connection.connect() # TODO 校验密码后连接到Redis Server的具体DB
            # connections that the pool provides should be ready to send
            # a command. if not, the connection was either returned to the
            # pool before all data has been read or the socket has been
            # closed. either way, reconnect and verify everything is good.
            # TODO 通过selector判断socket是否准备好读写，重试1次！！！
            if not connection.is_ready_for_command():  # TODO 通过使用selector的异步IO多路复用检测机制来判断socket是否已经准备好
    ...
    def make_connection(self):
        # TODO 仅仅创建了一个connection对象，并未建立socket链接
        "Create a new connection"

        if self._created_connections >= self.max_connections:  # TODO 检查是否达到链接数上限
    ...
    def release(self, connection):
        ...
        self._checkpid() # TODO 防止多线程并发重复release的问题
        if connection.pid != self.pid:
            return
        self._in_use_connections.remove(connection)  # TODO 从正在使用的队列中删除
        self._available_connections.append(connection)  # TODO 加到可使用的队列
