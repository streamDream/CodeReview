...
class Lock(object):
    ...
    def __init__(self, redis, name, timeout=None, sleep=0.1,
        ...
        TODO thread_local 控制不同的线程是否能否释放其他线程创建的相同name的锁
        """
        self.redis = redis
        self.name = name
        ...
        self.register_scripts()     # TODO 注册锁操作的script
    ...
    def acquire(self, blocking=None, blocking_timeout=None, token=None):
        ...
        sleep = self.sleep
        if token is None:
            token = uuid.uuid1().hex.encode()  # TODO 随机生成的token
        else:
    ...
    def do_acquire(self, token):
        if self.timeout:
            ...
            timeout = None
        if self.redis.set(self.name, token, nx=True, px=timeout):  # TODO 使用set()来实现锁
            return True
        return False
